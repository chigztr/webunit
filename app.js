...
function renderProductMedia(product){
  ...
  if(mediaType==='video'&&videoId){
    ...
    const fallbackAttr=!uploadedImage&&fallbackThumbnail?` onerror="this.onerror=null;this.src='${escapeHtml(fallbackThumbnail)}'"`:'';
...
function populateHeroProducts(products){
  ...
  stack.innerHTML=useful.map(p=>{
    ...
    if(!src) return '';
    return `<div class="hero-mini"><img src="${escapeHtml(src)}" alt=""><span>${escapeHtml(p.name)}</span></div>`;
  }).join('');
}
function enableClickToPlay(){
  document.addEventListener('click',event=>{
    ...
    if(!media) return;
    media.innerHTML=`<iframe class="youtube-iframe" src="https://www.youtube-nocookie.com/embed/${encodeURIComponent(videoId)}?autoplay=1&rel=0" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>`;
  });
}
